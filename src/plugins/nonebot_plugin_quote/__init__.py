...
from nonebot.adapters.red import Bot, Message, MessageEvent, PrivateMessageEvent, MessageSegment, GroupMessageEvent
...
@record_pool.handle()
async def record_pool_handle(bot: Bot, event: MessageEvent, state: T_State):
...
@record_help.handle()
async def record_help_handle(bot: Bot, event: MessageEvent, state: T_State):
...
async def delete_record_handle(bot: Bot, event: MessageEvent, state: T_State):
...
@alltag.handle()
async def alltag_handle(bot: Bot, event: MessageEvent, state: T_State):
...
@addtag.handle()
async def addtag_handle(bot: Bot, event: MessageEvent, state: T_State):
...
@deltag.handle()
async def deltag_handle(bot: Bot, event: MessageEvent, state: T_State):
...
@script_batch.handle()
async def script_batch_handle(bot: Bot, event: MessageEvent, state: T_State):
...
@copy_batch.handle()
async def copy_batch_handle(bot: Bot, event: MessageEvent, state: T_State):
